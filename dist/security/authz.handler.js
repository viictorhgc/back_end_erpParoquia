...
exports.authorize = (...profiles) => {
    return (req, resp, next) => {
        if (req.authenticated !== undefined && req.authenticated.hasAny(...profiles)) {
            req.log.debug('User %s is authorized with profiles %j on route %s. Required profiles: %j', req.authenticated._id, req.authenticated.profiles, req.path(), req.authenticated.profiles);
            next();
        }
        else {
